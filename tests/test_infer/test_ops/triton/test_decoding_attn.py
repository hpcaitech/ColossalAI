...
configs = [
    triton.testing.Benchmark(
        ...
        args={"bsz": BATCH, "block_size": BLOCK_SIZE, "same_context_len": SAME_LEN, "kv_group_num": 1, "q_len": 1},
    )
]


@triton.testing.perf_report(configs)
def bench_kernel(bsz, KV_LEN, provider, block_size: int, kv_group_num: int, same_context_len: bool, q_len: int):
    num_attn_heads = 16
    ...
    max_kv_len_in_b = kv_lengths.max().item()  # for random lengths
    ...
    if provider == "triton":
        ...
        # the maximum block length splitted on kv should be the kv cache block size
        kv_max_split_num = (max_kv_len_in_b + block_size - 1) // block_size
        output = torch.empty((bsz * q_len, num_attn_heads, HEAD_DIM), dtype=dtype, device=device)
        mid_output = torch.empty(
            size=(bsz * q_len, num_attn_heads, kv_max_split_num, HEAD_DIM), dtype=torch.float32, device=q.device
        )
        mid_output_lse = torch.empty(
...
if __name__ == "__main__":
    test_flash_decoding(16, 32, 32, 16, 1, True, 1)
