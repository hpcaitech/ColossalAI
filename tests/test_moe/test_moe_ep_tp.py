...
def run_test(rank: int, world_size: int, port: int, num_experts: int, batch_size: int, dim: int, seed: int):
    ...
    # sync tp param
    sync_tp_from_local(tp_model, local_model)
    ...
    out_tp = tp_model(input_data)
    MOE_MANAGER.reset_loss()
    out_ep = ep_model(shard_data)
    MOE_MANAGER.reset_loss()

    assert torch.allclose(out_tp, out_local, atol=1e-6), \
        f"Rank {rank} failed, max diff: {torch.max(torch.abs(out_tp - out_local))}"
    try:
        assert torch.allclose(out_ep, out_local[index:index + micro_batch_size], atol=1e-6), \
            f"Rank {rank} failed, max diff: {torch.max(torch.abs(out_ep - out_local[index:index + micro_batch_size]))}"
        ...
        """
        warnings.warn(
            "EP may result in different behavior from local model. "
    ...
    sync_tp_from_local(tp_model, local_model, assert_grad_flag=True)
    try:
