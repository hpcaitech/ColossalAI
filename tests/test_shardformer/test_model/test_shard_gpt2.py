...
def check_forward_backward(model_fn, data_gen_fn, output_transform_fn, loss_fn, test_config):
    ...
    if stage_manager is None or stage_manager.is_first_stage():
        check_weight(org_model.h[0].mlp.c_fc, sharded_model.h[0].mlp.c_fc, tp_group, atol=1e-5, rtol=1e-3)
        check_gradient(org_model.h[0].mlp.c_fc, sharded_model.h[0].mlp.c_fc, tp_group, atol=1e-5, rtol=1e-3)

    # check weights after optimizer.step()
    org_optimizer.step()
    sharded_optimizer.step()
    if stage_manager is None or stage_manager.is_first_stage():
        check_weight(org_model.h[0].mlp.c_fc, sharded_model.h[0].mlp.c_fc, tp_group, atol=5e-3, rtol=1e-3)
