...
def init_1d_row(weight, bias, pg: ProcessGroup):
    spec = ColoTensorSpec(distspec.shard([0], [pg.tp_world_size()]), ComputeSpec(ComputePattern.TP1D))
    with DistSpecManager.no_grad():
        weight.set_tensor_spec(spec)


def init_1d_col(weight, bias, pg: ProcessGroup):
    spec = ColoTensorSpec(distspec.shard([-1], [pg.tp_world_size()]), ComputeSpec(ComputePattern.TP1D))
    with DistSpecManager.no_grad():
        weight.set_tensor_spec(spec)
