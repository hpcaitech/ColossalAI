...
def test_sharding_spec():
    ...
    assert dim_spec_list_0[0].difference(dim_spec_list_1[0]) == ALLGATHER_COST + STEP_PENALTY + ALLGATHER_COST
    assert dim_spec_list_0[1].difference(dim_spec_list_1[1]) == SHARD_COST + STEP_PENALTY + SHARD_COST
    assert dim_spec_list_0[2].difference(dim_spec_list_1[2]) == 0
    assert dim_spec_list_0[3].difference(dim_spec_list_1[3]) == 0

    assert sharding_spec_0.sharding_sequence_difference(sharding_spec_1) == \
        reduce(operator.add, [dim_spec_list_0[i].difference(dim_spec_list_1[i]) for i in range(dims)], 0)
