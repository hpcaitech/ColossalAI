...
@parameterize('model_name', ['bert', 'gpt2', 'hanging_param_model', 'nested_model', 'repeated_computed_layers', 'simple_net'])
@parameterize('use_grad_checkpoint', [True])
def exam_gpt_fwd_bwd(placement_policy, keep_gather, model_name: str, use_grad_checkpoint: bool = False):
    ...
    gemini_non_model_data = gemini_manager._mem_stats_collector.non_model_data_list('cuda')
    print('gemini non model data:', gemini_non_model_data)
    runtime_tracer_non_model_data = GLOBAL_CUDA_MEM_INFO.non_model_data_list
