...
def check_ring_qk(rank, world_size):
    ...
    q = torch.rand(batch_size * num_heads, seq_length,
                   attention_head_size).cuda()
    k = torch.rand(batch_size * num_heads, seq_length,
                   attention_head_size).cuda()
    ...
    sub_q = q.clone()[:, rank * sub_seq_length:(rank + 1) *
                      sub_seq_length].contiguous()
    sub_k = k.clone()[:, rank * sub_seq_length:(rank + 1) *
                      sub_seq_length].contiguous()
    ...
    partial_master_a_grad = a.grad[:, rank * sub_seq_length:(rank + 1) *
                                   sub_seq_length]
    torch.autograd.backward(sub_a, partial_master_a_grad)

    # check master and distributed grads
    partial_master_q_grad = q.grad[:, rank * sub_seq_length:(rank + 1) *
                                   sub_seq_length]
...
def check_ring_av(rank, world_size):
    ...
    v = torch.rand(batch_size * num_heads, seq_length,
                   attention_head_size).cuda()
    ...
    # create distributed tensors
    sub_a = a.clone()[:, rank * sub_seq_length:(rank + 1) *
                      sub_seq_length].contiguous()
    sub_v = v.clone()[:, rank * sub_seq_length:(rank + 1) *
                      sub_seq_length].contiguous()
    ...
    sub_out = ring_av(sub_a, sub_v, batch_size, num_heads, attention_head_size,
                      sub_seq_length)
    ...
    # # run distributed backward
    partial_master_out_grad = out.grad[:, rank * sub_seq_length:(rank + 1) *
                                       sub_seq_length]
