...
from .msa import MSAColumnAttention, MSAColumnGlobalAttention, MSARowAttentionWithPairBias
...
from .triangular_attention import TriangleAttentionEndingNode, TriangleAttentionStartingNode
...
from .utils.tensor_utils import chunk_layer
...
class EvoformerBlockCore(nn.Module):
    ...
    def forward(
        ...
        z = z + self.ps_dropout_row_layer(self.tri_att_start(z, mask=pair_mask, chunk_size=chunk_size))
        z = z + self.ps_dropout_col_layer(self.tri_att_end(z, mask=pair_mask, chunk_size=chunk_size))
...
class ExtraMSABlock(nn.Module):
    ...
    def forward(
        ...
        def fn(m, z):
            m = m + self.msa_att_col(m, mask=msa_mask, chunk_size=chunk_size)
            m, z = self.core(m, z, msa_mask=msa_mask, pair_mask=pair_mask, chunk_size=chunk_size)
