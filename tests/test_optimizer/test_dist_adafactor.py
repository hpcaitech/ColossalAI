...
from colossalai.tensor.d_tensor import layout_converter
...
class TPModel(nn.Module):
    def __init__(self, linear1, linear2, tp_group=None):
        super().__init__()
        self.linear1 = Linear1D_Col.from_native_module(linear1, process_group=tp_group, gather_output=False, overlap=True)
...
def exam_dist_adafactor_fwd_bwd(dtype: torch.dtype):
    ...
    tp_model = TPModel(copy.deepcopy(base_model.linear1), copy.deepcopy(base_model.linear2), device_mesh.get_process_group(axis=1)).to(local_rank)
    tp_group = device_mesh.get_process_group(axis=1)
        ...
        if tp_shard_spec[id(tp_p)] is not None:
            if len(tp_shard_spec[id(tp_p)].sharding_sequence) >= 2:
                ...
                if tp_shard_spec[id(tp_p)].sharding_sequence[0] == 'R': 
                    tp_p = _gather(input_=tp_p.data.view(-1, HEIGHT // tensor_parallel_size),dim=-1, process_group=device_mesh.get_process_group(axis=1)) # gather
                # if tp_p tp_shard_spec is row tp  --> gather then view to (-1, H // tensor_parallel_size)
                else:
                    tp_p = _gather(input_=tp_p.data,dim=-1, process_group=device_mesh.get_process_group(axis=1)).view(-1, WIDTH) # gather
            else:
                # bias parallel
...
@parameterize("tp_zero_size", [(2, 2)])# (2, 2), (4, 1),(1, 4)
