...
def main(dtype):
    ...
    table = PrettyTable(['Version','Float Percision', 'iter', 'Device Nums', 'weight shape', 'bias shape', 'Avg runtime(ms)',
    ...
    table.add_row(["AdaFactor",dtype, niter, tensor_parallel_size, weight.shape, bias.shape, (base_runtime / niter) * 10.0**3, None, None])
    table.add_row(["DistributedAdaFactor (Col Parallel)", dtype, niter, tensor_parallel_size, weight.shape, bias.shape, (zero_runtime / niter) * 10.0**3, base_runtime/zero_runtime ,base_runtime/zero_best_runtime])
    ...
    table.add_row(["AdaFactor",dtype, niter , tensor_parallel_size, weight_row.shape, bias_row.shape, (base_runtime / niter) * 10.0**3, None, None])
    table.add_row(["DistributedAdaFactor (Row Parallel)",dtype, niter, tensor_parallel_size, weight_row.shape, bias_row.shape, (zero_runtime / niter) * 10.0**3, base_runtime/zero_runtime ,base_runtime/zero_best_runtime])
