...
def set_flat_grad(model: nn.Module, torch_model: nn.Module, g_dtype: torch.dtype) -> None:
    """Set grads for flattened weights in ZeRO stage 2"""
    pass
    # for p, torch_p in zip(model.parameters(), torch_model.parameters()):
...
def test_dist_lamb_optim_on_bert(
    optim_cls: DistributedLamb,
    device: torch.device,
    adamw: bool,
    p_dtype: torch.dtype,
    g_dtype: torch.dtype,
    ...
    eps = 1e-8
    torch_optim_cls = DistributedLamb
