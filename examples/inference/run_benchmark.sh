...
for input_len in  1024; do
    for output_len in 256; do
        for bsz in 32 64; do
            python3 ${PY_SCRIPT} -m llama2-7b --tp_size 1 --pp_size 1 -b ${bsz} -s ${input_len} --output_len ${output_len} --mode ${mode} --model_path /home/caidi/random_llama3/ | tee logs/${input_len}_${output_len}_${mode}_${GPU}_${bsz}.txt
        done
    done
done
