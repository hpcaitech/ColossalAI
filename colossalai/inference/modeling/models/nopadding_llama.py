...
def llama_model_forward(
    ...
    if use_cuda_kernel and inputmetadata.dtype != torch.float32 and use_flash_attn2:
...
class NopadLlamaMLP(LlamaMLP):
    def __init__(
        ...
        self.gate_up_weight = nn.Parameter(torch.stack([mlp_gproj_w, mlp_uproj_w], dim=0))
...
class TPNopadLlamaMLP(ParallelModule):
    def __init__(
        self,
        config: LlamaConfig,
        mlp_gproj_w: torch.Tensor = None,
    ...
    def from_native_module(
        ...
        mlp_layer = TPNopadLlamaMLP(
            ...
            # gather_output=module.gate_proj.gather_output,
            # *args,
            # **kwargs
        )
        del module
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        gate_up_proj_out = torch.bmm(hidden_states, self.gate_up_weight)
        act_out = inference_ops.silu_and_mul(gate_up_proj_out)
        # NOTE need gather here?
        # return torch.mm(act_out, self.down_proj_weight)
        return self.down_proj(act_out)
        # NOTE need reduce here?
...
class TPNopadLlamaAttention(ParallelModule):
    def __init__(
        self,
        config: LlamaConfig,
        layer_idx: Optional[int] = None,
    ...
    def from_native_module(
        ...
        attn_layer = TPNopadLlamaAttention(
            config=config,
            layer_idx=layer_idx,
            attn_qproj_w=attn_qproj_w,
            attn_kproj_w=attn_kproj_w,
    ...
    def forward(
        ...
        # print(f"token_nums, num_heads, head_dim: {token_nums}, {self.num_heads}, {self.head_dim}")
        logger.info(f"before the flash attn, shape is : {hidden_states.shape}")
        logger.info(f"the shape of qkv_weight is: {self.qkv_weight.shape}")
