    ...
    for i in range(bsz):
        _cache = torch.cat(
            (
                ...
                cache[block_tables[i][needed_blocks[i] - 1], :, :, : num_remaing_tokens[i]].reshape(
...
class PagedAttention(nn.Module):
    ...
    def nopad_context_forward(
        ...
        query, value = apply_rotary_pos_emb(query, key, cos, sin, position_ids)
