...
def _flash_decoding_fwd_kernel(
    ...
    mid_output,  # [batch_size, head_num, kv_split_num, head_dim]
    mid_output_lse,  # [batch_size, head_num, kv_split_num]
    ...
    offsets_n = block_start_kv * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    
    alibi_mask = tl.load(alibi + offsets_q)
    ...
    S_ij -= alibi_mask * (cur_kv_seq_len - 1 - offsets_n)
...
def _flash_decoding_fwd_reduce_kernel(
    ...
    for block_i in range(0, kv_split_num, 1):
        mid_o_block = tl.load(mid_output + offsets_mid_o + block_i * stride_mid_ob)
        lse = tl.load(mid_output_lse + offset_mid_lse + block_i * stride_o_lseb)
...
def flash_decoding_attention_with_alibi(
    q: torch.Tensor,
    ...
    if mid_output is None:
        mid_output = torch.empty(
            (bsz, num_heads, kv_max_split_num, head_dim), dtype=torch.float32, device=q.device
        )
        
    if mid_output_lse is None:
        mid_output_lse = torch.empty((bsz, num_heads, kv_max_split_num), dtype=torch.float32, device=q.device)
    
    if output is None:
        output = torch.empty((bsz, num_heads * head_dim), dtype=q.dtype, device=q.device)

    assert (
        mid_output.size(2) == mid_output_lse.size(2) >= kv_max_split_num
    ), "Incompatible kv split number of intermediate output tensors"
    assert (
        mid_output.size(0) == mid_output_lse.size(0) >= output.size(0) == bsz
