...
void dense_layer_fp16_forward(torch::Tensor& in, torch::Tensor& weight, torch::Tensor& out, int cublasAlgo = 99) {
        ...
        if(in.is_contiguous() == false){
            in = in.contiguous();
        }
        
        if(weight.is_contiguous() == false) { 
...
void batch_dense_layer_fp16_forward(torch::Tensor& in, torch::Tensor& weight, torch::Tensor& out, float alpha, float beta, bool weight_transpose = false, int cublasAlgo = 99) {
