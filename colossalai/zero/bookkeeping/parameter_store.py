...
class ParameterStore(BaseStore):
    ...
    def get_fp16_param_not_in_current_rank(self) -> List[Tensor]:
        param_list = []
        for param, rank in self._fp16_param_to_rank.items():
            if rank != self._local_rank:
                param_list.append(param)
        return param
    
    def get_fp16_param_in_current_rank(self) -> List[Tensor]:
        param_list = []
        for param, rank in self._fp16_param_to_rank.items():
            if rank == self._local_rank:
                param_list.append(param)
