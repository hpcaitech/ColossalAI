...
def convert_to_zero(model: nn.Module, optimizer: Optimizer, level: int,
                    zero_config: dict):
    """
    ...
    if level == 2:
        ...
        optimizer = ZeroRedundancyOptimizer_Level_3(init_optimizer=optimizer,
                                                    module=model,
                                                    **zero_config)
    return model, optimizer
...
def zero3_model_context(dtype=torch.half):
    ...
    ds_config = {
        ...
        "zero_optimization": {
            "offload_param":
            getattr(gpc.config.zero, 'offload_param_config', None),
            "offload_optimizer":
            getattr(gpc.config.zero, 'offload_optimizer_config'),
        },
        "aio": getattr(gpc.config.zero, 'aio_config', None)
    }
    remote_device = getattr(ds_config['zero_optimization']['offload_param'],
                            'device', None)
    pin_memory = getattr(ds_config['zero_optimization']['offload_param'],
                         'pin_memory', False)
    return deepspeed.zero.Init(data_parallel_group=gpc.get_group(
        ParallelMode.DATA),
...
__all__ = [
    'convert_to_zero', 'ZeroRedundancyOptimizer_Level_2',
