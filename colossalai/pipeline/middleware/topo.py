...
class Partition(object):
    ...
    def get_input_partition_ids(self, input_partition_id=None):
        res = []
        for input_val in self._input_vals:
            val_pos = input_val.get()
            if val_pos.partition_id not in res:
                if not input_partition_id \
                    or input_partition_id != val_pos.partition_id:
                    res.append(val_pos.partition_id)
        return res

    # get all output dst partition_ids
    def get_output_partition_ids(self, output_partition_id=None):
        res = []
        for output_val in self._output_vals:
            outputs = output_val.get()
            for val_pos in outputs:
                if val_pos.partition_id not in res:
                    if not output_partition_id \
                        or output_partition_id != val_pos.partition_id:
                        res.append(val_pos.partition_id)
        return res
