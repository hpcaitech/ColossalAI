...
def _shape_consistency_apply(gm: torch.fx.GraphModule):
    ...
    for node in nodes:
        ...
        for user_node_index, user_node in enumerate(node.strategies_vector.successor_nodes):
            ...
            #     MetaInfo(shape_consistency_node, mod_dir=user_node.meta['info'].mod_dir, activation_checkpoint=tuple(user_node.meta['info'].activation_checkpoint))
...
def _comm_spec_apply(gm: torch.fx.GraphModule):
    ...
    for node in nodes:
        ...
        for op_data, comm_action in comm_actions.items():
            ...
            #     MetaInfo(comm_spec_apply_node, mod_dir=node.meta['info'].mod_dir, activation_checkpoint=tuple(node.meta['info'].activation_checkpoint))
