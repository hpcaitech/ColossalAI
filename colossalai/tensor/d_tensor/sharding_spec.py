...
class ShardingSpec:
    """
    Sharding spec describes how to shard a tensor with dim_size dimensions. For example for a 4D tensor, the sharding sequence
    [R, S0, S1] means not sharding the first dim, sharding the 3rd along the 1st device mesh axis (Process group)
    and sharding the 4th dim along the 2nd device mesh axis. Useful for say, 2D Tensor Parallel.
