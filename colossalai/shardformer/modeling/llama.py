...
def test_llama_seq_parallel_attention():
    ...
    def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
        ...
        print_rank("cos-0", cos.shape)  # torch.Size([16, 32])
        print_rank("cos-position", cos[position_ids])  # torch.Size([1, 1, 4, 32])

        cos = cos[position_ids].unsqueeze(1)  # [bs, 1, seq_len, dim]
        sin = sin[position_ids].unsqueeze(1)  # [bs, 1, seq_len, dim]
        print_rank("cos-1", cos.shape)  # torch.Size([1, 1, 4, 32])
    ...
    def forward(
        ...
        print_rank("hidden_states-origin", hidden_states.shape)
        if self.config.pretraining_tp > 1:
        ...
        cos, sin = self.rotary_emb(value_states, seq_len=kv_seq_len)
        print_rank("cos", cos.shape)

        print_rank("position_ids", position_ids)
        if position_ids is not None:
            position_ids = torch.arange(
                past_key_value_length, q_len + past_key_value_length, dtype=torch.long, device=hidden_states.device
            )

        print_rank("position_ids-2", position_ids)
