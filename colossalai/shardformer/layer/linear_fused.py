...
__all__ = ['LinearFused1D_Col', 'LinearFused1D_Row']


def _rearange_tensor(tensor: torch.Tensor, process_group: ProcessGroup, n_fused: int, dim: int) -> torch.Tensor:
    ...
    weight_chunks = torch.chunk(tensor, world_size * n_fused, dim=dim)
    rearanged_weight_chunks = [weight_chunks[i] for i in new_order]
    rearanged_weight = torch.cat(rearanged_weight_chunks, dim=dim)

    return rearanged_weight


class LinearFused1D_Col(ParallelModule):
    ...
    def from_native_module(module: nn.Linear, process_group: Union[ProcessGroup, List[ProcessGroup]], *args,
        ...
        linear_1d = LinearFused1D_Col(in_features=in_features,
        ...
        with torch.no_grad():
            ...
            rearanged_weight = _rearange_tensor(module.weight.data, process_group, n_fused=n_fused, dim=0)
            sharded_weight = shard_rowwise(rearanged_weight, process_group)
            linear_1d.weight.data.copy_(sharded_weight)
            if bias:
                rearanged_bias = _rearange_tensor(module.bias.data, process_group, n_fused=n_fused, dim=0)
    ...
    def forward(self, input_: Tensor) -> Tuple[Tensor, Tensor]:
        assert input_.shape[-1] == self.weight.shape[-1], \
            'Invalid shapes in LinearFused1D_Col forward: input={}, weight={}. Expected last dim of input {}.'.format(
...
class LinearFused1D_Row(ParallelModule):
    ...
    def from_native_module(module: nn.Linear, process_group: Union[ProcessGroup, List[ProcessGroup]], *args,
        ...
        linear_1d = LinearFused1D_Row(in_features=in_features,
                                      out_features=out_features,
                                      bias=bias,
                                      device=device,
                                      process_group=process_group,
        ...
        with torch.no_grad():
            # the weigh to the linear layer is a transpose
            # thus shard on row is equal to shard on column
            rearanged_weight = _rearange_tensor(module.weight.data, process_group, n_fused=n_fused, dim=1)
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        if self.parallel_input:
            assert input_.shape[-1] == self.weight.shape[-1], \
                'Invalid shapes in LinearFused1D_Row forward: input={}, weight={}. Expected last dim of input {}.'.format(
                input_.shape, self.weight.shape, self.weight.shape[-1])
            input_ = input_
        else:
        ...
        if self.stream_chunk_num > 1:
            if self.training:
                raise RuntimeError("use stream_chunk_num=1 in LinearFused1D_Row for training!")
            with torch.no_grad():
