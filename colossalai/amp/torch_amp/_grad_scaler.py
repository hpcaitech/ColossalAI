    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def _unscale_grads_(self, optimizer, inv_scale, found_inf, allow_fp16):
        ...
        if gpc.is_initialized(ParallelMode.MODEL) and gpc.get_world_size(
            ...
            vals = [
                val
                for val in per_device_found_inf._per_device_tensors.values()
            ]
            coalesced = _flatten_dense_tensors(vals)
            dist.all_reduce(coalesced,
                            op=dist.ReduceOp.MAX,
                            group=gpc.get_group(ParallelMode.MODEL))
            for buf, synced in zip(vals,
                                   _unflatten_dense_tensors(coalesced, vals)):
                buf.copy_(synced)
    ...
    def step(self, optimizer, *args, **kwargs):
        ...
        retval = self._maybe_opt_step(optimizer, optimizer_state, *args,
                                      **kwargs)
