...
def is_using_pp():
    return gpc.is_initialized(ParallelMode.PIPELINE) and gpc.get_world_size(
        ParallelMode.PIPELINE) > 1


def is_using_sequence():
    return gpc.is_initialized(ParallelMode.SEQUENCE) and gpc.get_world_size(
        ParallelMode.SEQUENCE) > 1
...
def clip_grad_norm_fp32(parameters, max_norm, norm_type=2):
    ...
    if norm_type == inf:
        ...
        moe_parallel_grads = [
        ]  # used to collect moe tensor parallel gradients
        for p in params:
...
def param_is_not_tensor_parallel_duplicate(param):
    return (hasattr(param, IS_TENSOR_PARALLEL)
            and getattr(param, IS_TENSOR_PARALLEL)) or (gpc.get_local_rank(
