...
NON_COMPUTE_NAME = ["getattr", "getitem", "eq", "_assert_is_none", "_assert", "finfo", "size", "split", "type"]
...
def is_non_compute_node(node: Node) -> bool:
    if any(i in node.op for i in NON_COMPUTE_OP) or any(i in node.name for i in NON_COMPUTE_NAME):
        return True
