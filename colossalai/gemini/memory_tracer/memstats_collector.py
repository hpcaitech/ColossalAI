...
class MemStatsCollector:
    ...
    def next_period_non_model_data_usage(self, device_type: str) -> int:
        ...
        next_non_model_data = self._mem_stats.non_model_data_list(device_type)[self._step_idx]
    ...
    def sample_overall_data(self) -> None:
        ...
        if self._start_flag:
            ...
            if len(self._mem_stats._model_data_cuda_list) == 0:
                return

            self._mem_stats.append_overall_data('cuda', self._mem_monitor.finish())
            self._mem_stats.append_overall_data('cpu', colo_device_memory_used(torch.device('cpu')))

            assert len(self._mem_stats._model_data_cuda_list) == len(self._mem_stats._overall_cuda_list)

            self._mem_stats.append_non_model_data('cuda')
            self._mem_stats.append_non_model_data('cpu')
