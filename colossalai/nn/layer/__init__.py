...
_parallel_classifier = {'3d': Classifier3D}

_parallel_layernorm = {'1d': LayerNorm1D, '2d': LayerNorm2D, '2p5d': LayerNorm2p5D, '3d': LayerNorm3D}

_parallel_patchembedding = {'3d': PatchEmbedding3D}
...
class Linear(nn.Module):
    def __init__(self,
        ...
        if tensor_parallel is not None:
            self.layer = _parallel_linear[tensor_parallel](in_features, out_features, bias=bias, dtype=dtype)
        else:
...
class LayerNorm(nn.Module):
    def __init__(self, normalized_shape: int, eps=1e-05, dtype=None, tensor_parallel: Optional[str] = None) -> None:
        super().__init__()
        if tensor_parallel is not None:
            self.layer = _parallel_layernorm[tensor_parallel](normalized_shape, eps=eps, dtype=dtype)
        else:
            self.layer = nn.LayerNorm(normalized_shape, eps=eps, device=get_current_device(), dtype=dtype)
...
class PatchEmbedding(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        if tensor_parallel is not None:
            self.layer = _parallel_patchembedding[tensor_parallel](
                img_size,
                patch_size,
                in_chans,
                embed_size,
            ...
            self.layer = VanillaPatchEmbedding(
                img_size,
                patch_size,
                in_chans,
                embed_size,
