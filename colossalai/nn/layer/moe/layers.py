...
class Experts(nn.Module):
    ...
    moe tensor parallel group, where E is the number of experts. Every expert
    ...
    def __init__(self, expert, num_experts, **expert_args):
        ...
        assert num_experts % moe_env.tensor_parallel_size == 0, \
            "The number of experts should be divied by moe tensor size"

        num_local_experts = num_experts // moe_env.tensor_parallel_size
        with seed(ParallelMode.MOE_TENSOR):
...
class MoeLayer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        dispatch_data = AllToAll.apply(dispatch_data, ParallelMode.MOE_TENSOR)

        expert_output = self.experts(dispatch_data)

        expert_output = AllToAll.apply(expert_output, ParallelMode.MOE_TENSOR)
