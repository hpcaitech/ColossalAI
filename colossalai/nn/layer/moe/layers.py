...
class Top2Router(nn.Module):
    ...
    def get_capacity(self, logits_shape):
        capacity = math.floor(2 * self.capacity_factor *
                              logits_shape[-2] / logits_shape[-1])
        capacity += capacity % 2
        assert capacity > 0
        return capacity
    ...
    def forward(self, inputs: torch.Tensor, cuda_mode: bool = False):
        ...
        if cuda_mode:
            ...
            dest_idx = torch.stack([top1_idx * capacity + rank1,
                                    top2_idx * capacity + rank2], dim=0).to(torch.int32)
...
class MoeLayer(nn.Module):
    ...
    def expert_part(self, expert_input: torch.Tensor):
        ...
        expert_input = expert_input.reshape(moe_env.model_parallel_size,
                                            self.num_experts // moe_env.model_parallel_size,
                                            -1, self.d_model)
