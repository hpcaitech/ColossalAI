...
class MoeLayer(nn.Module):
    ...
    def __init__(self, dim_model: int, num_experts: int, router: MoeRouter, experts: BaseExperts):
        super().__init__()
...
class MoeModule(nn.Module):
    ...
    def __init__(self,
        ...
        moe_router = moe_router_cls(capacity_factor_train=capacity_factor_train,
        ...
        # moe residual
        # TODO: update residual algorithm according to switch transformer
        ...
        if use_residual:
            if residual_instance is not None:
                ...
                self.residual_module = expert_cls(hidden_size=hidden_size,
                                                  intermediate_size=intermediate_size,
                                                  activation=activation)

            self.residual_combine = nn.Linear(hidden_size, 2, device=get_current_device())

        # moe experts
        expert_cls = get_expert_class(expert_parallel)
        moe_experts = expert_cls(num_experts=num_experts,
                                 hidden_size=hidden_size,
                                 intermediate_size=intermediate_size,
                                 activation=activation)

        # combine experts and router in moe layer
        self.moe_layer = MoeLayer(dim_model=hidden_size,
