...
class MoeExperts(nn.Module):
    ...
    def __init__(self, comm: str, num_experts: int):
        super().__init__()
        assert comm in {"all_to_all", "all_gather"}, \
            "This kind of communication has not been implemented yet.\n Please use Experts build function."
        self.comm = comm
        # Get the configuration of experts' deployment and parallel information from moe_env
        self.num_local_experts, self.dist_info = moe_env.get_info(num_experts)
...
class TPExperts(MoeExperts):
    ...
    def __init__(self, num_experts: int, d_model: int, d_ff: int, activation=None, drop_rate: float = 0):
        super().__init__("all_gather", moe_env.max_ep_size)

        assert d_ff % moe_env.max_ep_size == 0, \
            "d_ff should be divied by maximum expert parallel size"

        p_ff = d_ff // moe_env.max_ep_size
