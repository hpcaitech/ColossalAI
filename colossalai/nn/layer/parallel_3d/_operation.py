...
class linear_3d(torch.autograd.Function):
    ...
    def backward(ctx, output_grad: Tensor) -> Tuple[Tensor, ...]:
        ...
        with torch.no_grad():
            ...
            input_grad, op = reduce_scatter(input_grad,
                                            ctx.input_dim,
            ...
            weight_grad, op = all_reduce(weight_grad,
            ...
            if ctx.use_bias:
                ...
                bias_grad, op = all_reduce(bias_grad,
                                           ctx.weight_parallel_mode,
                                           async_op=True)
                async_ops.append(op)
...
class classifier_3d(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd(cast_inputs=torch.float16)
    def forward(ctx, input_: Tensor, weight: Tensor, bias: Optional[Tensor],
                input_parallel_mode: ParallelMode,
                weight_parallel_mode: ParallelMode,
    ...
    def backward(ctx, output_grad: Tensor) -> Tuple[Tensor, ...]:
        ...
        with torch.no_grad():
            ...
            weight_grad = reduce(weight_grad, ctx.src_rank,
                                 ctx.input_parallel_mode)
...
class layernorm_3d(torch.autograd.Function):
    ...
    def forward(ctx, input_: Tensor, weight: Tensor, bias: Tensor,
    ...
    def backward(ctx, output_grad: Tensor) -> Tuple[Tensor, ...]:
        ...
        with torch.no_grad():
            ...
            dvar = all_reduce(torch.sum(dvar, dim=-1, keepdim=True),
                              ctx.output_parallel_mode)
            dmean = dz * (-1 / sigma) + dvar * -2 * mu / ctx.normalized_shape
            dmean = all_reduce(torch.sum(dmean, dim=-1, keepdim=True),
                               ctx.output_parallel_mode)
...
def split_tensor_3d(
    ...
    if input_.size(dim) <= 1:
        return input_
    output = torch.chunk(
        input_, gpc.get_world_size(weight_parallel_mode),
        dim=dim)[gpc.get_local_rank(weight_parallel_mode)].contiguous()
    output = torch.chunk(
        output, gpc.get_world_size(input_parallel_mode),
...
class reduce_by_batch_3d(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        if reduce_mean:
            reduce_size = gpc.get_world_size(
                input_parallel_mode) * gpc.get_world_size(weight_parallel_mode)
...
class broadcast_weight_3d_from_diagonal(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd(cast_inputs=torch.float16)
    def forward(ctx, input_: Tensor, input_parallel_mode: ParallelMode,
                weight_parallel_mode: ParallelMode,
                output_parallel_mode: ParallelMode) -> Tensor:
        ranks_in_group = gpc.get_ranks_in_group(input_parallel_mode)
        src_rank = ranks_in_group[gpc.get_local_rank(output_parallel_mode)]
    ...
    def backward(ctx, output_grad: Tensor) -> Tuple[Tensor, ...]:
        ...
        if gpc.get_local_rank(ctx.input_parallel_mode) == gpc.get_local_rank(
                ctx.output_parallel_mode):
