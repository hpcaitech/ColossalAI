...
class LayerNorm3D(ParallelLayer):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        return layernorm_3d.apply(input_, self.weight, self.bias,
                                  self.normalized_shape, self.variance_epsilon,
...
class Linear3D(ParallelLayer):
    ...
    def reset_parameters(self, weight_initializer, bias_initializer) -> None:
        with seed(ParallelMode.TENSOR):
            ...
            weight_src_rank = gpc.get_ranks_in_group(
                self.weight_parallel_mode)[0]
            output_src_rank = gpc.get_ranks_in_group(
                self.output_parallel_mode)[0]
            ...
            if self.bias is not None:
                ...
                broadcast(self.bias, weight_src_rank,
                          self.weight_parallel_mode)
                broadcast(self.bias, output_src_rank,
                          self.output_parallel_mode)
...
class Classifier3D(ParallelLayer):
    ...
    def reset_parameters(self, weight_initializer, bias_initializer) -> None:
        with seed(ParallelMode.TENSOR):
            ...
            output_src_rank = gpc.get_ranks_in_group(
                self.output_parallel_mode)[0]
            input_src_rank = gpc.get_ranks_in_group(
                self.input_parallel_mode)[0]

            if self.has_weight:
                weight_initializer(self.weight, fan_in=fan_in, fan_out=fan_out)
                broadcast(self.weight, weight_src_rank,
                          self.weight_parallel_mode)
    ...
    def forward(self, input_: Tensor) -> Tensor:
        return classifier_3d.apply(input_, self.weight, self.bias,
                                   self.input_parallel_mode,
                                   self.weight_parallel_mode,
                                   self.output_parallel_mode)
...
class PatchEmbedding3D(ParallelLayer):
    ...
    def __init__(
        ...
        self.bias = nn.Parameter(
            torch.empty(embed_size_per_partition,
                        device=get_current_device(),
                        dtype=dtype))
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros((1, num_patches + 1, embed_size_per_partition),
                        device=get_current_device(),
                        dtype=dtype))
    ...
    def reset_parameters(self, weight_initializer, bias_initializer,
                         position_embed_initializer) -> None:
        with seed(ParallelMode.TENSOR):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        weight = broadcast_weight_3d_from_diagonal.apply(
            self.weight, self.input_parallel_mode, self.weight_parallel_mode,
            self.output_parallel_mode)
