...
class LayerNorm3D(ParallelLayer):
    ...
    def __init__(self, normalized_shape: int, eps: float = 1e-12, device=None, dtype=None):
        super().__init__()
        ...
        self.weight = Parameter(torch.ones(self.normalized_shape_per_partition, device=device, dtype=dtype))
        self.bias = Parameter(torch.zeros(self.normalized_shape_per_partition, device=device, dtype=dtype))
...
class Linear3D(ParallelLayer):
    ...
    def __init__(self,
        ...
        self.weight = Parameter(
            torch.empty(self.in_features_per_partition, self.out_features_per_partition, device=device, dtype=dtype))
        if bias:
            self.bias = Parameter(torch.zeros(self.out_features_per_partition, device=device, dtype=dtype))
        else:
            self.bias = None
...
class Classifier3D(ParallelLayer):
    ...
    def __init__(self,
        ...
        if weight is not None:
            ...
            self.weight = Parameter(
                torch.empty(self.num_classes, self.in_features_per_partition, device=device, dtype=dtype))
...
class VocabParallelClassifier3D(ParallelLayer):
    ...
    def __init__(self,
        ...
        if weight is not None:
            ...
            self.weight = Parameter(
                torch.empty(self.out_features_per_partition, self.in_features_per_partition, device=device,
                            dtype=dtype))
            self.has_weight = True
        if bias:
            self.bias = Parameter(torch.zeros(self.out_features_per_partition, device=device, dtype=dtype))
    ...
    def forward(self, input_: Tensor) -> Tensor:
        return linear_3d.apply(input_, self.weight.transpose(0, 1), self.bias, self.input_parallel_mode,
...
class PatchEmbedding3D(ParallelLayer):
    ...
    def __init__(self,
        ...
        self.weight = nn.Parameter(
            torch.empty((embed_size_per_partition, in_chans, *self.patch_size), device=device, dtype=dtype))
        self.bias = nn.Parameter(torch.empty(embed_size_per_partition, device=device, dtype=dtype))

        self.cls_token = nn.Parameter(torch.zeros((1, 1, embed_size_per_partition), device=device, dtype=dtype))
        self.pos_embed = nn.Parameter(
            torch.zeros((1, num_patches + 1, embed_size_per_partition), device=device, dtype=dtype))
...
class Embedding3D(ParallelLayer):
    ...
    def __init__(self,
        ...
        self.weight = nn.Parameter(torch.empty((num_embeddings, embed_dim_per_partition), device=device, dtype=dtype))
...
class VocabParallelEmbedding3D(torch.nn.Module):
    ...
    def __init__(self,
        ...
        tensor_parallel_rank = gpc.get_local_rank(self.output_parallel_mode)
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        output = reduce_tensor_3d.apply(output_parallel, self.output_parallel_mode)
