...
class _AllGatherTensor2D(torch.autograd.Function):
    ...
    def forward(ctx: Any, inputs: Tensor, dim: int, parallel_mode: ParallelMode) -> Tensor:
        ctx.dim = dim
        ctx.summa_dim = env.summa_dim
        ctx.row_rank = gpc.get_local_rank(parallel_mode)
...
def split_tensor_2d(tensor: Tensor, dim: int, parallel_mode: ParallelMode) -> Tensor:
    ...
    if tensor.size(dim) <= 1:
        return tensor
    return torch.chunk(tensor, env.summa_dim, dim=dim)[gpc.get_local_rank(parallel_mode)].contiguous()
