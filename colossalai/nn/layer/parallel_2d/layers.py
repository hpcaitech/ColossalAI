...
from ._operation import Matmul_AB_2D, Matmul_ABT_2D, add_bias_2d, all_gather_weight_2d, classifier_2d, layernorm_2d, reduce_tensor_2d
...
class VocabParallelEmbedding2D(torch.nn.Module):
    ...
    def __init__(self,
        ...
        tensor_parallel_rank = gpc.get_local_rank(ParallelMode.PARALLEL_2D_ROW)
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        output = reduce_tensor_2d.apply(output_parallel, ParallelMode.PARALLEL_2D_ROW)
        return output
...
class VocabParallelClassifier2D(ParallelLayer):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.bias is not None:
            output = add_bias_2d.apply(output, self.bias, self.hidden_size_per_partition, self.row_rank, self.col_rank,
