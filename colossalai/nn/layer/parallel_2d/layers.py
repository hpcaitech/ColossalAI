...
from colossalai.communication.collective import reduce_scatter
...
class Embedding2D(ParallelLayer):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        output = all_gather_tensor_2d(output, -1, ParallelMode.PARALLEL_2D_COL)
        output = split_tensor_2d(output, 0, ParallelMode.PARALLEL_2D_COL)
        return output
...
class VocabParallelEmbedding2D(torch.nn.Module):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        output = reduce_scatter_tensor_2d(output_parallel, ParallelMode.PARALLEL_2D_COL)
        return output
