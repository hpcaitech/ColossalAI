...
class RingQK(torch.autograd.Function):
    ...
    def forward(ctx, sub_q, sub_k, batch_size, num_attention_heads,
        ...
        for i in range(local_world_size - 1):
            ...
            start_idx, end_idx = _calc_incoming_device_range(
                i, local_rank, local_world_size, sub_seq_length)
            part_a = torch.matmul(sub_q, sub_k.transpose(2, 1))
            attention_score[:, :, start_idx:end_idx] = part_a
    ...
    def backward(ctx, grad_output):
        ...
        grad_k = grad_k[:, local_rank * ctx.sub_seq_length:(local_rank + 1) *
                        ctx.sub_seq_length]
        ...
        start_idx, end_idx = _calc_current_device_range(
            local_rank, ctx.sub_seq_length)
        ...
        for i in range(local_world_size - 1):
            sub_k = ring_forward(sub_k, ParallelMode.SEQUENCE)
            start_idx, end_idx = _calc_incoming_device_range(
                i, local_rank, local_world_size, ctx.sub_seq_length)
            grad_q += torch.matmul(grad_output[:, :, start_idx:end_idx], sub_k)
...
class RingAV(torch.autograd.Function):
    ...
    def forward(ctx, attention_score, sub_v, batch_size, num_attention_heads,
        ...
        local_rank = gpc.get_local_rank(ParallelMode.SEQUENCE)
        local_world_size = gpc.get_world_size(ParallelMode.SEQUENCE)
        local_start_idx, local_end_idx = _calc_current_device_range(
            local_rank, sub_seq_length)
        ...
        # compute local AV
        part_av = torch.matmul(
            attention_score[:, :, local_start_idx:local_end_idx], sub_v)
        ...
        for i in range(local_world_size - 1):
            ...
            # compute QK^T
            part_av = torch.matmul(attention_score[:, :, start_idx:end_idx],
                                   sub_v)
            sub_attention_result += part_av
    ...
    def backward(ctx, grad_output):
        ...
        grad_attention_score[:, :,
                             local_start_idx:local_end_idx] += torch.matmul(
                                 grad_output, sub_v.transpose(2, 1))
