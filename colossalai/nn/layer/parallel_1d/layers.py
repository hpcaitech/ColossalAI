...
from ._utils import (gather_forward_split_backward, get_parallel_input,
                     reduce_grad, reduce_input, set_parallel_input,
...
class Classifier1D(ParallelLayer):
    ...
    def __init__(
        ...
        self.input_size_per_partition = divide(in_features,
                                               gpc.tensor_parallel_size)
        ...
        if bias:
            self.bias = Parameter(
                torch.empty(self.num_classes, **factory_kwargs))
        else:
            self.bias = None
    ...
    def reset_parameters(self, weight_initializer, bias_initializer) -> None:
        ...
        if self.bias is not None:
            ...
            broadcast(self.bias,
                      gpc.get_ranks_in_group(ParallelMode.PARALLEL_1D)[0],
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        if self.parallel_input:
            ...
            input_ = split_forward_gather_backward(input_,
                                                   ParallelMode.PARALLEL_1D,
                                                   dim=-1)
...
class Linear1D_Col(ParallelLayer):
    ...
    def __init__(
        ...
        self.out_features_per_partition = divide(out_features,
                                                 gpc.tensor_parallel_size)
    ...
    def forward(self, input_: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if self.gather_output:
            ...
            output = gather_forward_split_backward(output_parallel,
                                                   ParallelMode.PARALLEL_1D,
                                                   dim=-1)
        else:
...
class MixedFusedLayerNorm1D(torch.nn.Module):
    ...
    def forward(self, input):
        return FusedLayerNormAffineFunction1D.apply(input, self.weight,
                                                    self.bias,
...
class Embedding1D(ParallelLayer):
    ...
    def __init__(self,
        ...
        embed_dim_per_partition = divide(embedding_dim,
                                         gpc.tensor_parallel_size)
    ...
    def forward(self, input_: Tensor) -> Tensor:
        ...
        output_parallel = F.embedding(input_, self.weight, self.padding_idx,
...
class Dropout1D(ParallelLayer):
    ...
    def forward(self, input_: Tensor) -> Tensor:
        cm = nullcontext() if not self.parallel_input else seed(
            ParallelMode.TENSOR)
        with cm:
