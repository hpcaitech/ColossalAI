...
class PipelineSchedule(BaseSchedule):
    ...
    def pre_processing(self, engine):
        if isinstance(
                engine.optimizer,
            (ZeroRedundancyOptimizer_Level_2, ZeroRedundancyOptimizer_Level_3)):
            raise TypeError(
    ...
    def _call_engine(model, input_tensor, batch_data):
        ...
        if isinstance(batch_data, torch.Tensor):
            ...
            if filter_batch:
                batch_data = {
                    k: v for k, v in batch_data.items() if k in sig.parameters
                }
            if input_tensor is None:
                return model(**batch_data)
            else:
    ...
    def forward_backward_step(self,
        ...
        for k in range(num_microbatches_remaining):
            ...
            if gpc.is_pipeline_last_stage(ignore_virtual=True):
                ...
                next_backward_model_chunk_id = get_model_chunk_id(backward_k +
                                                                  1,
                                                                  forward=False)
