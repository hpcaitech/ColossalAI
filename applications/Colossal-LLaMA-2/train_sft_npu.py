...
# NPU flash attention
from attn import SUPPORT_FLASH, NPULlamaAttention, NPURMSNorm, replace_xformers
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="gemini",
        choices=["gemini", "gemini_auto", "fsdp", "fsdp_cpu", "zero2", "zero2_cpu", "3d"],
    ...
    coordinator.print_on_master(
        f"Max NPU memory after data loader: {torch.npu.max_memory_allocated() / 1024 ** 2:.2f} MB"
    )
    ...
    # ======================================================
    init_ctx = (
        LazyInitContext(default_device=get_current_device()) if isinstance(plugin, (GeminiPlugin, HybridParallelPlugin)) else nullcontext()
    )
    ...
    # whether the model is sharded or not
    model_files = os.listdir(args.pretrained)
    has_index_json = any(file.endswith('.index.json') for file in model_files)

    if args.load_checkpoint is None:
        if not has_index_json:
            pth = os.path.join(args.pretrained, "pytorch_model.bin")
            coordinator.print_on_master(f"Load pretrained model checkpoint from {pth}")
            booster.load_model(model, pth, strict=False)
        else:
    ...
    coordinator.print_on_master(f"Booster init max NPU memory: {torch.npu.max_memory_allocated() / 1024 ** 2:.2f} MB")
    coordinator.print_on_master(
    ...
    if args.load_checkpoint is not None:
        ...
        coordinator.print_on_master(
            f"Checkpoint loaded max NPU memory: {torch.npu.max_memory_allocated() / 1024 ** 2:.2f} MB"
        )
        coordinator.print_on_master(
            f"Checkpoint loaded NPU memory: {torch.npu.memory_allocated() / 1024 ** 2:.2f} MB"
        )
        coordinator.print_on_master(
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        total_loss = torch.tensor(0.0).to(f"npu:{torch.npu.current_device()}")
    ...
    coordinator.print_on_master(f"Max NPU memory usage: {torch.npu.max_memory_allocated()/1024**2:.2f} MB")
