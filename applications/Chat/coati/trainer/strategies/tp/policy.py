...
class Linear1DColPolicy(Policy):
    ...
    def replace(self, module: nn.Module) -> bool:
        ...
        module.weight.data = module.weight.chunk(self.tp_size, dim=0)[self.tp_rank]
        if module.bias is not None:
            if isinstance(module.bias, LazyTensor):
                module.bias.materialize()
            module.bias.data = module.bias.chunk(self.tp_size, dim=0)[self.tp_rank]
        # replace forward
...
class Linear1DRowPolicy(Policy):
    ...
    def replace(self, module: nn.Module) -> bool:
        ...
        if isinstance(module.weight, LazyTensor):
            module.weight.materialize()
        module.weight.data = module.weight.chunk(self.tp_size, dim=1)[self.tp_rank]
