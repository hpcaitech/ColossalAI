...
class ZeroDPStrategy(NaiveStrategy):
    ...
    def setup_distributed(self) -> None:
        # original tp group is now zero group
        config = dict(parallel=dict(tensor=dict(
            mode='1d',
            size=self.zero_size,
        )))
        colossalai.launch_from_torch(config, seed=self.seed)
        self.zero_optim_config['zero_process_group'] = gpc.get_group(ParallelMode.PARALLEL_1D)
        self.zero_optim_config['dp_process_group'] = gpc.get_group(ParallelMode.DATA)
    ...
    def save_optimizer(self, optimizer: Optimizer, path: str, only_rank0: bool = False) -> None:
        if gpc.get_local_rank(ParallelMode.DATA) == 0:
            path = f'{path}.r{gpc.get_local_rank(ParallelMode.PARALLEL_1D)}'
