...
class EnglishRetrievalConversation:
    ...
    def __init__(self, retriever: CustomRetriever, model_path: str, model_name: str) -> None:
        ...
        self.llm_chain_disambiguate = LLMChain(llm=self.llm, prompt=PROMPT_DISAMBIGUATE_EN, 
        ...
        self.memory = ConversationBufferWithSummary(llm=self.llm, 
                                    llm_kwargs={'max_new_tokens':50, 'temperature':0.6, 'do_sample':True})
        self.memory.initiate_document_retrieval_chain(self.llm, 
                                    PROMPT_RETRIEVAL_QA_EN, self.retriever, chain_type_kwargs={'chat_history':'', })
        self.retrieval_chain = RetrievalQA.from_chain_type(llm=self.llm, verbose=False, chain_type="stuff", 
                retriever=self.retriever, chain_type_kwargs={"prompt": PROMPT_RETRIEVAL_QA_EN,"memory":self.memory },
    ...
    @classmethod
    def from_retriever(cls, retriever: CustomRetriever, model_path: str, model_name: str) -> "EnglishRetrievalConversation":
