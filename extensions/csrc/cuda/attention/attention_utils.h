...
template <>
struct VecType<at::BFloat16, 2> {
  using Type = __nv_bfloat162;
};
template <>
struct VecType<at::BFloat16, 4> {
  using Type = bfloat164;
};
template <>
struct VecType<at::BFloat16, 8> {
  using Type = bfloat168;
};

template <>
struct VecType<at::Half, 2> {
  using Type = half2;
};
template <>
struct VecType<at::Half, 4> {
  using Type = half4;
};
template <>
struct VecType<at::Half, 8> {
  using Type = half8;
};
...
inline __device__ void from_float(bfloat164& dst, float4_ src) {
  ...
  dst.y = __floats2bfloat162_rn(src.y);
#endif
}
...
inline __device__ void from_float(bfloat168& dst, float8_ src) {
  ...
  dst.x = __float22bfloat162_rn(src.x);
  dst.y = __floats2bfloat162_rn(src.y);
  dst.z = __floats2bfloat162_rn(src.z);
  dst.w = __floats2bfloat162_rn(src.w);
#endif
}
...
template <>
inline __device__ void fma(__nv_bfloat16 a, __nv_bfloat16 b, float& c) {
  c = __bfloat162float(a) * __bfloat162float(b) + c;
}

template <>
inline __device__ void fma(half a, half b, float& c) {
  c = __half2float(a) * __half2float(b) + c;
}

// Q*K^T operation.
template <int NUM_THREADS_PER_TOKEN, typename Vec>
inline __device__ float qk_dot_(const Vec& q, const Vec& k) {
