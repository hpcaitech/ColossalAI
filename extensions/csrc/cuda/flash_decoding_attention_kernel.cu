...
template<int NUM_WARPS, NUM_THREADS_PER_GROUP=1>
...
__global__ void flash_decoding_attention_kernel(
  ...
  const int* __restrict__ block_tables,       // [num_tokens, max_seq_len_per_seq]
  ...
  constexpr int NUM_TOKEN_PER_ROUND = VEC_SIZE * WARP_SIZE / HEAD_SIZE;
  constexpr int NUM_THREADS_PER_TOKEN = HEAD_SIZE / VEC_SIZE;
  ...
  for (int block_idx = warp_idx; block_idx < num_context_blocks; block_idx += NUM_WARPS) {
    ...
    #pragma unroll
    for (int idx = lane; idx < BLOCK_SIZE * HEAD_SIZE / VEC_SIZE; idx += WARP_SIZE) {
      const int token_idx = block_idx * BLOCK_SIZE + idx / NUM_THREADS_PER_TOKEN;
      const cache_t* k_ptr = k_cache + physical_block_number * kv_block_stride
                                     + kv_head_idx * kv_head_stride
                                     + idx * VEC_SIZE;
      K_vec k_vec = *reinterpret_cast<const K_vec*>(k_ptr);
      float qk = scale * Qk_dot<scalar_t, NUM_THREADS_PER_TOKEN>::dot(((K_vec*)(q_shared_ptr + idx * VEC_SIZE % HEAD_SIZE))[0], k_vec);
  ...
  for (int block_idx = warp_idx; block_idx < num_context_blocks; block_idx += NUM_WARPS) {
    ...
    for (int idx = lane; idx < BLOCK_SIZE * HEAD_SIZE / VEC_SIZE; idx += WARP_SIZE) {
      ...
      const cache_t* v_ptr = v_cache + physical_block_number * kv_block_stride
                                     + kv_head_idx * kv_head_stride
                                     + idx * VEC_SIZE;
      V_vec v_vec = *reinterpret_cast<const V_vec*>(v_ptr);
      ...
      #pragma unroll
      for (int i = 0; i < VEC_SIZE; i++) {
        fma(logit, ((scalar_t*) &v_vec)[i], accs[i]);
      }
    }
  }
  ...
  #pragma unroll
  for (int i = 0; i < VEC_SIZE; i++) {
    accs[i] = block_sum<NUM_WARPS, NUM_THREADS_PER_TOKEN>(out_shared_mem, accs[i]);
  }

  if (thread_idx < NUM_THREADS_PER_TOKEN) {
    scalar_t* out_ptr = out + seq_idx * q_stride + head_idx * HEAD_SIZE;
    for (int i = 0; i < VEC_SIZE; i++) {
      from_float(((scalar_t*) &zero_value)[0], accs[i]);
    }
    ((L_vec*) out_ptr)[thread_idx] = (L_vec)zero_value;
  }
}
...
void flash_decoding_attention_v1_launcher(
  ...
  torch::Tensor& block_tables,     // [num_tokens, max_seq_len]
  int max_context_len,
  float scale) {
...
void flash_decoding_attention(
  ...
  switch (query.scalar_type()) {
    ...
    case at::ScalarType::Half:
      CALL_V1_LAUNCHER_BLOCK_SIZE(at::Half, at::Half);
      break;
    case at::ScalarType::BFloat16:
      CALL_V1_LAUNCHER_BLOCK_SIZE(at::BFloat16, at::BFloat16);
      break;
    default:
