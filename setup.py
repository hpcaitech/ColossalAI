    ...
    if CUDA_HOME is None:
        ...
        def cuda_ext_helper(name, sources, extra_cuda_flags):
            return CUDAExtension(
                name=name,
                sources=[
                    os.path.join('colossalai/kernel/cuda_native/csrc', path)
                    for path in sources
                ],
                include_dirs=[
                    os.path.join(
                ...
                extra_compile_args={
                    ...
                    'nvcc':
                    append_nvcc_threads(['-O3', '--use_fast_math'] +
                                        version_dependent_macros +
                                        extra_cuda_flags)
                })
        ...
        ext_modules.append(
            cuda_ext_helper('colossal_C', [
                'colossal_C_frontend.cpp', 'multi_tensor_sgd_kernel.cu',
                'multi_tensor_scale_kernel.cu', 'multi_tensor_adam.cu',
                'multi_tensor_l2norm_kernel.cu', 'multi_tensor_lamb.cu'
            ], ['-lineinfo']))
        ...
        extra_cuda_flags = [
            '-std=c++14', '-U__CUDA_NO_HALF_OPERATORS__',
        ...
        ext_modules.append(
            cuda_ext_helper('colossal_multihead_attention', [
                'multihead_attention_1d.cpp', 'kernels/cublas_wrappers.cu',
                'kernels/transform_kernels.cu', 'kernels/dropout_kernels.cu',
                'kernels/normalize_kernels.cu', 'kernels/softmax_kernels.cu',
